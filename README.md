HELAL GIDA SORGULAMA VE BİLGİLENDİRME SİSTEMİ  

243301070 - Fatma Olgun

1. Yönetici (Admin) Hesabı
 E-posta: fatma@gmail.com
 Şifre: 123456
 Yetki Seviyesi: Ürün arama, barkod tarama, favorilere ekleme, profil ayarları, yeni ürün ekleme, ürün düzenleme, ürün silme ve kendi log kaydını görüntüleme.

 2. Kullanıcı Hesabı
 E-posta: ela@gmail.com
 Şifre: ela123
 Yetki Seviyesi: Ürün arama, barkod tarama, favorilere ekleme, profil ayarları, kendi log kaydını görüntüleme.

 Kullanılan Paketler
 `supabase_flutter`: Kullanıcı kimlik doğrulama (Auth), ilişkisel veri tabanı yönetimi ve anlık log insert işlemleri için.
 `mobile_scanner`: Cihaz kamerasını kullanarak gerçek zamanlı barkod ve QR kod tarama işlemleri için.
 `google_fonts`: Uygulama içi tipografi ve estetik arayüz tasarımı için.

 Uygulamanın Ekran Görüntüleri
<table>
  <tr>
    <td><img src="https://github.com/user-attachments/assets/e160c044-4776-4e0f-b577-83aae2cffa5e" width="200" alt="ekran1"/></td>
    <td><img src="https://github.com/user-attachments/assets/0e47d06d-c72c-43af-97a6-825efe12bfa3" width="200" alt="ekran2"/></td>
    <td><img src="https://github.com/user-attachments/assets/cac742cb-c274-4ac4-9b29-59ec1a121bf5" width="200" alt="ekran3"/></td>
    <td><img src="https://github.com/user-attachments/assets/7a244090-c2c6-4db0-ba75-302a584ddc5a" width="200" alt="ekran4"/></td>
  </tr>
  <tr>
    <td><img src="https://github.com/user-attachments/assets/c09ce048-26e5-4938-b2f3-369bb64ba91e" width="200" alt="ekran5"/></td>
    <td><img src="https://github.com/user-attachments/assets/fa4a1816-9418-42f3-9cd3-e69c9e06d819" width="200" alt="ekran6"/></td>
    <td><img src="https://github.com/user-attachments/assets/031f950e-aeb7-4c9c-82f9-060327a1334d" width="200" alt="ekran7"/></td>
    <td><img src="https://github.com/user-attachments/assets/241a872e-860c-47d9-98ae-1e6a53501bdb" width="200" alt="ekran8"/></td>
  </tr>
</table>






 
